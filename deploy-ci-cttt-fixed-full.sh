...
# This script runs the Cloud Build CI/CD CTTT pipeline with proper IP substitutions
...
# Connect to GKE cluster and get IP addresses
echo -e "${YELLOW}[CONFIG] Connecting to Kubernetes cluster to retrieve IP addresses${NC}"
...
# Try to get the STAGING_IP from the Kubernetes service
echo -e "${BLUE}[INFO] Retrieving STAGING_IP from Kubernetes service${NC}"
...
# Check if STAGING_IP was successfully retrieved
if [ -z "$STAGING_IP" ]; then
    echo -e "${YELLOW}[WARNING] Could not retrieve STAGING_IP from Kubernetes service${NC}"
    echo -e "${YELLOW}[WARNING] Using placeholder IP address for STAGING_IP${NC}"
    # Use a placeholder IP if we couldn't get the real one
    STAGING_IP="0.0.0.0"
else
    echo -e "${GREEN}[INFO] Successfully retrieved STAGING_IP: $STAGING_IP${NC}"
fi

# Try to get the LOAD_BALANCER_IP from the Kubernetes service
echo -e "${BLUE}[INFO] Retrieving LOAD_BALANCER_IP from Kubernetes service${NC}"
LOAD_BALANCER_IP=$(kubectl get svc aixtiv-cli -n anthology-ai -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)

# Check if LOAD_BALANCER_IP was successfully retrieved
if [ -z "$LOAD_BALANCER_IP" ]; then
    echo -e "${YELLOW}[WARNING] Could not retrieve LOAD_BALANCER_IP from Kubernetes service${NC}"
    echo -e "${YELLOW}[WARNING] Using placeholder IP address for LOAD_BALANCER_IP${NC}"
    # Use a placeholder IP if we couldn't get the real one
    LOAD_BALANCER_IP="0.0.0.0"
else
    echo -e "${GREEN}[INFO] Successfully retrieved LOAD_BALANCER_IP: $LOAD_BALANCER_IP${NC}"
fi

# Set substitution variables
SUBSTITUTIONS="_DEPLOY_ENV=staging,_TRIGGER_SOURCE=manual,STAGING_IP=$STAGING_IP,LOAD_BALANCER_IP=$LOAD_BALANCER_IP"
echo -e "${BLUE}[INFO] Using substitutions: $SUBSTITUTIONS${NC}"
...
# Submit the build with IP addresses as substitution variables
gcloud builds submit --config="$CONFIG_FILE" --substitutions="$SUBSTITUTIONS" .
