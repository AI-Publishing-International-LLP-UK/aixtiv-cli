...
const REQUIRED_FIELDS_V1 = ['id', 'name', 'version', 'description', 'capabilities', 'status'];
  ...
  try {
    ...
    if (version === 'v1') {
      ...
      if (
        ...
        !Array.isArray(card.agentProfile.capabilities)
      ) {
    ...
    if (version === 'v1') {
      ...
      if (card.status && !validStatuses.includes(card.status)) {
        result.issues.push(
          `Invalid status: ${card.status}. Must be one of: ${validStatuses.join(', ')}`
        );
        result.fixable = true;
      }
    }
    ...
    if (version === 'v1') {
      ...
      if (
        card.agentProfile &&
        card.agentProfile.version &&
        !/^\d+\.\d+\.\d+$/.test(card.agentProfile.version)
      ) {
        result.issues.push(
          `Invalid version format in agentProfile: ${card.agentProfile.version}. Should be in format x.y.z`
        );
        result.fixable = true;
      }
      ...
      if (
        ...
        !/^\d+\.\d+\.\d+$/.test(card.metadata.version)
      ) {
        result.issues.push(
          `Invalid version format in metadata: ${card.metadata.version}. Should be in format x.y.z`
        );
        result.fixable = true;
      }
    }
    ...
    if (card.team_assignments && Array.isArray(card.team_assignments)) {
      for (let i = 0; i < card.team_assignments.length; i++) {
        ...
        if (!team.team_id || !team.role) {
          result.issues.push(
            `Team assignment at index ${i} is missing required fields (team_id, role)`
          );
          result.fixable = false;
        }
      }
    ...
    if (version === 'v1') {
      if (card.id && !card.id.includes(expectedIdPart.replace('dr-', ''))) {
        result.issues.push(
          `ID "${card.id}" does not match expected pattern from filename "${expectedIdPart}"`
        );
        result.fixable = true;
      }
    } else {
      ...
      if (
        card.agentProfile &&
        card.agentProfile.id &&
        !card.agentProfile.id.includes(
          expectedIdPart.replace('dr-', '').replace('drmatch-', 'dr-match-')
        )
      ) {
        result.issues.push(
          `ID "${card.agentProfile.id}" does not match expected pattern from filename "${expectedIdPart}"`
        );
        result.fixable = true;
      }
    }
    ...
    if (result.issues.length === 0) {
      ...
      if (options.verbose) {
        if (version === 'v1') {
          ...
          console.log(
            `  Brain Architecture: ${Object.keys(card.brainArchitecture).length} components`
          );
      ...
      console.log(
        `${chalk.red('✗')} ${chalk.bold(fileName)}: Invalid (${result.issues.length} issues)`
      );
  ...
  try {
    ...
    if (version === 'v1') {
      ...
      if (!card.agentProfile) {
        ...
        if (
          card.agentProfile.id &&
          !card.agentProfile.id.includes(normalizedPart.replace('dr-', ''))
        ) {
