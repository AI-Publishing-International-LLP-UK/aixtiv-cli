...
async function extractAgentDefinitions() {
  try {
    ...
    if (!Array.isArray(agents) || agents.length === 0) {
      console.error(`${COLORS.red}No agents found in the JSON file or invalid format${COLORS.reset}`);
      throw new Error('No agents found in the JSON file or invalid format');
    }
    
    console.log(`${COLORS.green}Successfully loaded ${agents.length} agents from JSON file${COLORS.reset}`);
    ...
    console.error(`${COLORS.red}Failed to extract agent definitions: ${err.message}${COLORS.reset}`);
  ...
  try {
    ...
    for (const file of cardFiles) {
      try {
        ...
        console.error(`${COLORS.red}Error reading agent card ${file}: ${err.message}${COLORS.reset}`);
        changeLog.errors.push({
...
function updateAgentCard(existingCard, formalAgent) {
  ...
  updated.capabilities = [
    ...new Set([...newCapabilities, ...existingCapabilities])
      ...
      if (existingCard) {
        ...
        try {
          ...
          if (JSON.stringify(updatedCard) !== JSON.stringify(existingCard.content)) {
            ...
            console.log(`${COLORS.yellow}Updated agent card: ${existingCard.filename}${COLORS.reset}`);
            ...
            console.log(`${COLORS.cyan}No changes needed for: ${existingCard.filename}${COLORS.reset}`);
          }
        } catch (err) {
          console.error(`${COLORS.red}Error updating agent card ${agent.id}: ${err.message}${COLORS.reset}`);
          changeLog.errors.push({
            type: 'update_error',
            id: agent.id,
            message: err.message
        ...
        try {
          ...
          console.error(`${COLORS.red}Error creating agent card ${agent.id}: ${err.message}${COLORS.reset}`);
          changeLog.errors.push({
            type: 'creation_error',
            id: agent.id,
            message: err.message
