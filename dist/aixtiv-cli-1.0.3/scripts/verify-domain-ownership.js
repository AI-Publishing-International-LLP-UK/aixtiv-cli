...
function verifyDomainOwnership(godaddyDomainsFile) {
  ...
  try {
    ...
    if (!fs.existsSync(godaddyDomainsFile)) {
      console.error(
        `${COLORS.red}GoDaddy domains file not found: ${godaddyDomainsFile}${COLORS.reset}`
      );
      return {
    ...
    console.log(
      `${COLORS.green}Found ${godaddyDomains.length} domains in GoDaddy file${COLORS.reset}`
    );
    ...
    console.log(
      `${COLORS.cyan}Domains in cache:          ${COLORS.white}${cachedDomains.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Domains in GoDaddy:        ${COLORS.white}${godaddyDomains.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Verified domains (keep):   ${COLORS.green}${verifiedDomains.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Unverified domains (remove): ${COLORS.red}${unverifiedDomains.length}${COLORS.reset}`
    );
    ...
    console.log(
      `${COLORS.green}Verified domains saved to: ${VERIFIED_DOMAINS_OUTPUT}${COLORS.reset}`
    );
    console.log(
      `${COLORS.green}Unverified domains saved to: ${UNVERIFIED_DOMAINS_OUTPUT}${COLORS.reset}`
    );
    ...
    if (unverifiedDomains.length > 0) {
      console.log(
        `${COLORS.yellow}Found ${unverifiedDomains.length} domains in the cache that are not verified in GoDaddy.${COLORS.reset}`
      );
      ...
      if (unverifiedDomains.length > sampleSize) {
        console.log(
          `${COLORS.yellow}... and ${unverifiedDomains.length - sampleSize} more${COLORS.reset}`
        );
      }
      ...
      console.log(
        `${COLORS.white}2. Use the verified-domains.txt file with bulk-domain-import.sh${COLORS.reset}`
      );
    } else {
      console.log(
        `${COLORS.green}All domains in the cache are verified in GoDaddy!${COLORS.reset}`
      );
    }
...
if (require.main === module) {
  ...
  if (process.argv.length < 3) {
    ...
    console.error(
      `${COLORS.yellow}Usage: node verify-domain-ownership.js [godaddy-domains-file]${COLORS.reset}`
    );
    process.exit(1);
  }
