...
async function createProfile(userId, options) {
  ...
  try {
    ...
    console.log(chalk.grey(`  aixtiv copilot speaker --action enroll --profileId ${result.profileId} --file path/to/audio.wav --phrase "your enrollment phrase"`));
...
async function enrollSpeaker(profileId, filePath, phrase, options) {
  ...
  try {
    ...
    if (result.enrollmentComplete) {
      ...
      console.log(chalk.grey(`  aixtiv copilot speaker --action verify --profileId ${profileId} --file path/to/verification-audio.wav`));
    } else {
      console.log(chalk.cyan('\nContinue Enrollment:'));
      console.log('Additional voice samples needed. Add another sample using:');
      console.log(chalk.grey(`  aixtiv copilot speaker --action enroll --profileId ${profileId} --file path/to/next-audio.wav --phrase "another enrollment phrase"`));
    }
  ...
  try {
    ...
    if (result.identified && result.matches && result.matches.length > 0) {
      ...
      if (result.matches.length > 1) {
        ...
        const table = result.matches.slice(1, 4).map((match, index) => {
      ...
      console.log(boxen(
        chalk.yellow('No speaker identified with sufficient confidence'),
...
async function listProfiles(userId) {
  ...
  try {
    ...
    if (profiles.length === 0) {
      console.log(boxen(
        chalk.yellow(`No speaker profiles found for user ${userId}`),
        { padding: 1, borderColor: 'yellow', margin: 1 }
      ));
      return [];
    }
    ...
    profiles.forEach((profile, index) => {
      const statusColor = 
        profile.enrollmentStatus === 'completed' ? 'green' :
        profile.enrollmentStatus === 'in_progress' ? 'yellow' : 'blue';
      ...
      console.log(boxen(
        ...
        { padding: 1, borderColor: statusColor, margin: { top: index > 0 ? 0 : 1, bottom: 0, left: 1, right: 1 } }
      ));
    });
    
    // Provide next steps
    console.log('\nTo see details for a specific profile:');
    console.log(chalk.grey(`  aixtiv copilot speaker --action profile-details --profileId <profile-id>`));
...
async function getProfileDetails(profileId) {
  ...
  try {
    ...
    if (profile.enrolledPhrases && profile.enrolledPhrases.length > 0) {
      ...
      const phrasesText = profile.enrolledPhrases.map((enrollment, idx) => {
    ...
    if (profile.recentVerifications && profile.recentVerifications.length > 0) {
      ...
      const verificationsText = profile.recentVerifications.map((verification, idx) => {
...
async function deleteProfile(profileId) {
  ...
  try {
    ...
    console.log(boxen(
      chalk.bold(`Profile ${profileId} has been permanently deleted`),
