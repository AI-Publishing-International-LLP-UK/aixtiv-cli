...
async function createPreview(userId, copilotId, message, response, options) {
  ...
  try {
    const preview = await previewService.createPreview(userId, copilotId, message, response, options);
  ...
  try {
    ...
    console.log(
      boxen(
        ...
        (updatedPreview.approvalNote ? `${chalk.green('Note:')} ${updatedPreview.approvalNote}` : ''),
...
async function manageSettings(userId, options) {
  ...
  if (!options.showEmotionIndicators && !options.showToneSuggestions && 
      !options.showAIThinking && !options.transparencyLevel) {
...
async function submitFeedback(previewId, userId, feedbackType, comment) {
  if (!previewId || !userId || !feedbackType) {
    ...
    console.log('  --previewId     Preview ID');
    console.log('  --userId        User ID');
    console.log('  --feedbackType  Feedback type (helpful, not-helpful, tone-issue, needs-improvement)');
    return;
  }
  ...
  try {
    ...
    if (preview.messageSentiment) {
      const sentimentColor = 
        preview.messageSentiment.category === 'positive' ? 'green' :
        preview.messageSentiment.category === 'negative' ? 'red' : 'yellow';
    ...
    if (preview.aiThinking && preview.settings.showAIThinking) {
      ...
      console.log(
        boxen(
          ...
          preview.aiThinking.steps.map(step => 
            `• ${chalk.blue(step.step)}: ${step.description}\n  ${step.details}`
          ).join('\n\n') + '\n\n' +
    ...
    statusInfo.push(`${chalk.bold('Created:')} ${preview.timestamp ? preview.timestamp.toDate().toLocaleString() : 'Unknown'}`);
    statusInfo.push(`${chalk.bold('Status:')} ${preview.approved ? 'Approved' : (preview.changesRequested ? 'Changes Requested' : (preview.edited ? 'Edited' : 'Pending'))}`);
    
    if (preview.edited) {
      statusInfo.push(`${chalk.bold('Edited:')} ${preview.editedAt ? preview.editedAt.toDate().toLocaleString() : 'Unknown'}`);
    }
    
    if (preview.approved) {
      statusInfo.push(`${chalk.bold('Approved:')} ${preview.approvedAt ? preview.approvedAt.toDate().toLocaleString() : 'Unknown'}`);
    ...
    if (preview.changesRequested) {
      statusInfo.push(`${chalk.bold('Changes Requested:')} ${preview.changeRequestedAt ? preview.changeRequestedAt.toDate().toLocaleString() : 'Unknown'}`);
