...
function identifyDomainFamily(domain) {
  ...
  if (baseDomain.includes('learn') || baseDomain.includes('academy') || baseDomain.includes('training')) {
    return 'learning';
  }
  ...
  if (baseDomain.includes('shop') || baseDomain.includes('store') || baseDomain.includes('market')) {
    return 'commerce';
  }
...
async function organizeDomains(options = {}) {
  console.log(boxen(gradient.pastel(' Domain Family Organization '), 
  ...
  try {
    ...
    spinner.succeed(`Organized ${domains.length} domains into ${Object.keys(families).length} families`);
      ...
      for (const family of Object.keys(families)) {
        if (!PROJECT_MAPPINGS[family]) {
          ...
          if (oldProject !== project) {
            console.log(chalk.yellow(`Reassigning family "${family}" from ${oldProject} to ${project}`));
    ...
    if (!options.dryRun) {
      ...
      console.log(chalk.yellow('To apply these assignments, run the command without the --dry-run flag.'));
    }
  } catch (error) {
