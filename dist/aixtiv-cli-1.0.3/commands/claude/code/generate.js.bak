...
module.exports = async function generateCode(options) {
  ...
  try {
    ...
    logAgentAction('code_generation_result', {
      ...
      language,
      agent_id: getCurrentAgentId(),
      generator_used: result.isLocalGenerator ? 'local' : result.isLocalFallback ? 'fallback' : 'api',
    });
    ...
    displayResult({
      ...
      message: `Code generation ${result.status === 'completed' ? 'successfully completed' : 'failed'}${
        result.isLocalGenerator ? ' (using local generator)' : 
        result.isLocalFallback ? ' (using basic fallback)' : ''
      }`,
      details: {
        task: task,
        language,
