...
async function accessSecret(options) {
  ...
  try {
    ...
    if (exportToFile) {
      const exportPath = typeof exportToFile === 'string' 
        ? exportToFile 
      ...
      if (redact) {
        console.log(`Value: ${secretValue.substring(0, 3)}${'*'.repeat(Math.max(0, secretValue.length - 6))}${secretValue.substring(secretValue.length - 3)}`);
      }
    } else if (redact) {
      // Print redacted value
      console.log(`Value: ${secretValue.substring(0, 3)}${'*'.repeat(Math.max(0, secretValue.length - 6))}${secretValue.substring(secretValue.length - 3)}`);
...
async function deleteSecret(options) {
  ...
  try {
    ...
    spinner.succeed(`${version ? `Version ${version} of secret` : 'Secret'} ${secretId} deleted successfully`);
  } catch (error) {
...
async function rotateServiceAccountKey(options) {
  ...
  try {
    ...
    if (dryRun) {
      spinner.info(`DRY RUN: Would rotate key for ${serviceAccountEmail} and store in secret ${secretId}`);
      return;
    }
...
module.exports = async function secrets(options) {
  ...
  switch (action) {
    ...
    default:
      ...
      console.log('Available actions: list, create, get, delete, rotate-sa-key, rotate-api-key, setup-rotation, generate, audit');
  }
};