...
try {
  ...
  logAgentAction = (action, details) => {
    console.log(
      chalk.dim(`[No Agent Tracking] Action: ${action} | Details: ${JSON.stringify(details)}`)
    );
...
const ENV_SCHEMA = {
  ...
  FIREBASE_CONFIG: {
    ...
    validator: (value) => {
      try {
        ...
        return config && typeof config === 'object' && config.projectId && config.apiKey;
      } catch (e) {
        return false;
      }
    },
  ...
  LANGCHAIN_01_KEY: {
    required: false,
    errorType: ERROR_TYPES.WARNING,
    description: 'LangChain 01 API key',
    validator: (value) => {
      // Just ensure it's a non-empty string
      return !!value;
    },
  ...
  LANGCHAIN_02_KEY: {
    required: false,
    errorType: ERROR_TYPES.WARNING,
    description: 'LangChain 02 API key',
    validator: (value) => {
      // Just ensure it's a non-empty string
      return !!value;
    },
  ...
  CROSS_VALIDATIONS: [
    ...
    },
    {
      ...
      validator: (values) => {
        ...
        if (
          values.CLAUDE_API_ENDPOINT &&
          values.CLAUDE_API_ENDPOINT.includes('api.anthropic.com')
        ) {
...
function validateEnvironment(displayOutput = true) {
  ...
  if (displayOutput) {
    ...
    if (results.isValid && results.warnings === 0) {
      ...
      console.log(
        chalk.yellow(
          `⚠ Passed with ${results.warnings} warning${results.warnings !== 1 ? 's' : ''}`
        )
      );
    } else {
      console.log(
        chalk.red(
          `✖ Failed with ${results.criticalErrors} critical error${results.criticalErrors !== 1 ? 's' : ''}`
        ) +
          (results.warnings
            ? chalk.yellow(` and ${results.warnings} warning${results.warnings !== 1 ? 's' : ''}`)
            : '')
      );
    }
  }
