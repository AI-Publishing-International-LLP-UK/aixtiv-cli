...
async function removeDomainCommand(domain, options, { spinner } = {}) {
  ...
  try {
    ...
    console.log(
      'Note: This does not affect domain registration or DNS settings at your registrar.'
    );
  } catch (error) {
...
async function listDomainsCommand(options, { spinner } = {}) {
  ...
  try {
    ...
    const cacheExpired =
      cacheAge > (options.cacheTtl || utils.config.get('performance.cacheTTL', 3600)) * 1000;
...
function registerCommands(register) {
  ...
  register(
    ...
    [
      ...
      {
        flags: '--status <status>',
        description: 'Filter by status (active, pending, transferring)',
      },
