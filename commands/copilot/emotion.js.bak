...
command
  ...
  .action(async function () {
    try {
      ...
      console.log(
        `Contextual Tone Adjustment: ${preferences.contextualToneEnabled ? chalk.green('Enabled') : chalk.red('Disabled')}`
      );
      ...
      if (Object.keys(preferences.domainSpecificTones).length > 0) {
        ...
        for (const [domain, settings] of Object.entries(preferences.domainSpecificTones)) {
          console.log(
            `${chalk.yellow(domain)}: ${settings.tone} (Intensity: ${settings.intensity}/10)`
          );
        }
      }
    ...
    try {
      ...
      const answers = await inquirer.prompt([
        ...
        },
        {
          ...
          validate: (value) =>
            value >= 1 && value <= 10 ? true : 'Please enter a number between 1 and 10',
        },
        {
          type: 'confirm',
      ...
      if (answers.contextualToneEnabled) {
        console.log(
          chalk.yellow(
            'Contextual tone adjustment is enabled. Your copilot may adjust its tone based on conversation context.'
          )
        );
      } else {
        console.log(
          chalk.yellow(
            'Contextual tone adjustment is disabled. Your copilot will always use your preferred tone.'
          )
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.originalSentiment) {
        console.log(
          '\n' +
            chalk.dim(
              `Original message sentiment: ${result.originalSentiment.category} (${result.originalSentiment.score.toFixed(2)})`
            )
        );
      }
  ...
  .action(async function () {
    try {
      ...
      if (suggestion.sentiment) {
        console.log(
          chalk.dim(
            `Message sentiment: ${suggestion.sentiment.category} (${suggestion.sentiment.score.toFixed(2)})`
          )
        );
      }
  ...
  .action(async function () {
    try {
      ...
      console.log(
        '\n' +
          chalk.green(
            `Your custom tone "${answers.name}" has been created and added to your preferences.`
          )
      );
      console.log(
        chalk.yellow(
          'Note: Custom tones can be selected just like built-in tones when using the emotion set command.'
        )
      );
    } catch (error) {
  ...
  .action(async function () {
    try {
      ...
      console.log(
        '\n' +
          chalk.green(
            `Your copilot will now use a ${answers.tone} tone with intensity level ${answers.intensity}/10 for the domain "${answers.domain}".`
          )
      );
      console.log(
        chalk.yellow(
          'Note: Contextual tone adjustment must be enabled for domain-specific tones to take effect.'
        )
      );
    } catch (error) {
