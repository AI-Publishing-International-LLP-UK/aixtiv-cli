...
module.exports = async function copilotExpiration(options) {
  ...
  if (!email && !latest) {
    displayResult({
      success: false,
      message: 'Principal email is required (--email) or use --latest to target most recent relationship'
    });
    return;
  }
  ...
  if (!copilot && !latest) {
    displayResult({
      success: false,
      message: 'Co-pilot email or ID is required (--copilot) or use --latest to target most recent relationship'
    });
    return;
  }
  ...
  try {
    ...
    if (latest) {
      ...
      const relationships = await firestore.collection('copilotRelationships')
    ...
    console.log(chalk.yellow('      The principal will be prompted to review the relationship when the review date is reached.'));
