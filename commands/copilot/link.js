...
module.exports = async function copilotLink(options) {
  ...
  if (!validLevels.includes(level)) {
    displayResult(`Error: Invalid co-pilot level. Valid options are: ${validLevels.join(', ')}`, 'error');
    return;
  }
  ...
  if (!pilotDomainPattern.test(copilotEmail)) {
    console.log(chalk.yellow(`Warning: ${copilotEmail} does not match ASOOS pilot domain pattern.`));
  ...
  try {
    ...
    const result = await withSpinner(
      `Linking co-pilot ${copilotEmail} to principal ${email}`,
