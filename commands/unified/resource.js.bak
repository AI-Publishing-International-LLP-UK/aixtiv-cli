...
async function scanResources(options) {
  ...
  try {
    ...
    // Scan resources
    const result = await withSpinner('Scanning resources', scanUnifiedResources, filters);
    ...
    if (result.success) {
      ...
      const tableData = [['Resource ID', 'Name', 'Authorized Agents', 'Authorized Principals']];
      ...
      if (resource && result.resources.length === 1) {
        ...
        const accessTableData = [['Agent', 'Principal', 'Access Type', 'Granted At']];
