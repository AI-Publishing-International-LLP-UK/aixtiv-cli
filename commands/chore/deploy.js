...
const deployCommand = new Command('deploy')
  ...
  .option('-t, --type <type>', 'Deployment type (dual-interface, single, minimal)', 'dual-interface')
...
<body>
  ...
  <p>Mode: <span id="mode-switch">Standard</span></p>
          ...
          fs.writeFileSync(path.join(v1Path, 'index.html'), defaultHtmlContent.replace('{{VERSION}}', 'v1'));
          fs.writeFileSync(path.join(v2Path, 'index.html'), defaultHtmlContent.replace('{{VERSION}}', 'v2'));
          ...
          const updateHtmlWithScript = (htmlPath) => {
            if (fs.existsSync(htmlPath)) {
              ...
              if (!htmlContent.includes('mode-switch.js')) {
                htmlContent = htmlContent.replace('</body>', '  <script src="/js/mode-switch.js"></script>\n</body>');
...
exit 0`;
        ...
        if (options.dnsCheck) {
          ...
          try {
            ...
            if (failedDomains.length === 0) {
              ...
              spinner.warn(`DNS check passed for ${successfulDomains.length}/${domains.length} domains`);
        ...
        deploymentSteps.forEach(step => {
          console.log(chalk.green(` - ${step.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}`));
        });
      ...
      if (currentStep) {
        console.error(chalk.red(`Failed during step: ${currentStep.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}`));
      }
