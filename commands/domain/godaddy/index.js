...
const commands = {
  ...
  list: async (options = {}) => {
    console.log(
      boxen(gradient.pastel(' GoDaddy Domains '), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'cyan',
    ...
    try {
      ...
      if (options.expired) {
        filteredDomains = filteredDomains.filter(
          (domain) => domain.expires && getDaysRemaining(domain.expires) <= 0
        );
      } else if (options.expiring) {
        filteredDomains = filteredDomains.filter(
          (domain) =>
            domain.expires &&
            getDaysRemaining(domain.expires) <= 30 &&
            getDaysRemaining(domain.expires) > 0
        );
      }
      ...
      filteredDomains.forEach((domain) => {
        table.addRow({
          ...
          status:
            domain.status === 'ACTIVE' ? chalk.green(domain.status) : chalk.yellow(domain.status),
  ...
  status: async (domainName, options = {}) => {
    ...
    try {
      ...
      console.log(
        boxen(gradient.pastel(` Domain Status: ${domainName} `), {
          padding: 1,
          margin: 1,
          borderStyle: 'round',
          borderColor: 'cyan',
      ...
      table.addRow({
        property: 'Auto-Renew',
        value: domain.autoRenew ? chalk.green('Enabled') : chalk.red('Disabled'),
      });
      table.addRow({
        property: 'Privacy',
        value: domain.privacy ? chalk.green('Enabled') : chalk.red('Disabled'),
      });
      table.addRow({
        property: 'Locked',
        value: domain.locked ? chalk.green('Yes') : chalk.red('No'),
      });
      table.addRow({
        property: 'Transfer Protected',
        value: domain.transferProtected ? chalk.green('Yes') : chalk.red('No'),
      });
  ...
  sync: async (options = {}) => {
    console.log(
      boxen(gradient.pastel(' GoDaddy to Firebase Sync '), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'cyan',
  ...
  dns: async (domainName, options = {}) => {
    ...
    console.log(
      boxen(gradient.pastel(` DNS Management: ${domainName} `), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'cyan',
...
function registerGoDaddyCommands(program) {
  ...
  program
    ...
    .option(
      '-s, --status <status>',
      'Filter by domain status (ACTIVE, PENDING, CANCELLED)',
      'ACTIVE'
    )
