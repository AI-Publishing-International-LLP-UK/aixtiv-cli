  ...
  try {
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        console.error(
          chalk.red('The domain may not have a valid SSL certificate or might not be accessible.')
        );
...
async function checkAllSSL(options) {
  ...
  if (cache.domains.length === 0) {
    console.log(
      chalk.yellow('No domains found in the cache. Add domains using aixtiv domain add <domain>')
    );
    return;
  }
...
async function provisionFirebaseSSL(domain, project) {
  ...
  exec('firebase --version', async (error) => {
    ...
    exec(
      `firebase hosting:sites:list --project ${project} | grep -q "${siteName}" || firebase hosting:sites:create ${siteName} --project ${project}`,
      (error) => {
        if (error) {
          console.error(
            chalk.yellow(`Warning: Could not create site ${siteName}: ${error.message}`)
          );
        }
        ...
        exec(
          `firebase hosting:domain:add ${domain} --site ${siteName} --project ${project}`,
          (error, stdout, stderr) => {
            if (error) {
              console.error(chalk.yellow(`Warning: ${stderr}`));
              console.log(
                chalk.blue(
                  'You may need to verify domain ownership. Follow the instructions from Firebase.'
                )
              );
            } else {
...
async function provisionGCPSSL(domain, project) {
  ...
  exec('gcloud --version', async (error) => {
    ...
    exec(
      `gcloud config set project ${project} && gcloud compute ssl-certificates create ${certName} --domains=${domain} --global`,
      (error, stdout, stderr) => {
        if (error) {
          console.error(chalk.yellow(`Warning: ${stderr}`));
          console.log(
            chalk.blue('You may need to update your load balancer to use this certificate.')
          );
        } else {
