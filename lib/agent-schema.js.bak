...
async function createAgent(agentData) {
  try {
    ...
    const allCapabilities = [...squadronCapabilities, ...typeCapabilities, ...customCapabilities];
  ...
  try {
    ...
    if (updates.humanCollaborator !== undefined) {
      ...
      if (oldCollaborator) {
        ...
        const oldCollaboratorRef = firestore
          .collection('humanCollaborators')
          .doc(oldCollaboratorId);
      ...
      if (updates.humanCollaborator) {
        ...
        const newCollaboratorRef = firestore
          .collection('humanCollaborators')
          .doc(newCollaboratorId);
  ...
  try {
    ...
    if (extension.newType && AGENT_TYPES[extension.newType]) {
      ...
      updates.capabilities = [
        ...new Set([...agent.capabilities, ...typeCapabilities, ...(extension.capabilities || [])]),
      ];
    } else if (extension.capabilities) {
      // Just add the new capabilities if not changing type
      updates.capabilities = [
        ...new Set([...agent.capabilities, ...(extension.capabilities || [])]),
      ];
    }

    // Add specialties
    if (extension.specialties) {
      updates.specialties = [...new Set([...agent.specialties, ...extension.specialties])];
    }
  ...
  try {
    ...
    for (const agentId of agentIds) {
      try {
        ...
        const squadron = 'S04';
        const number =
          parseInt(agentId.replace(/\D/g, '')) || agentIds.size + results.migrated.agents + 1;
