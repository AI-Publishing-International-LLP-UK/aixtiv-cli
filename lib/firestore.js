...
// Helper functions for Firestore FieldValue operations with fallbacks
// These provide safe access to Firestore FieldValue methods even in test environments

/**
 * Safe wrapper for serverTimestamp that provides a fallback
 * @returns {Object} A server timestamp or fallback
 */
function getServerTimestamp() {
  ...
  console.warn('Warning: serverTimestamp not available, using fallback');
  return new Date();
}

/**
 * Safe wrapper for arrayUnion that provides a fallback
 * @param {...any} elements - Elements to add to the array
 * @returns {Object} Array union operation or fallback
 */
function getArrayUnion(...elements) {
  if (admin.firestore && admin.firestore.FieldValue && admin.firestore.FieldValue.arrayUnion) {
    return admin.firestore.FieldValue.arrayUnion(...elements);
  }
  // Fallback for testing environments
  console.warn('Warning: arrayUnion not available, using fallback');
  return elements;
}

/**
 * Safe wrapper for arrayRemove that provides a fallback
 * @param {...any} elements - Elements to remove from the array
 * @returns {Object} Array remove operation or fallback
 */
function getArrayRemove(...elements) {
  if (admin.firestore && admin.firestore.FieldValue && admin.firestore.FieldValue.arrayRemove) {
    return admin.firestore.FieldValue.arrayRemove(...elements);
  }
  // Fallback for testing environments
  console.warn('Warning: arrayRemove not available, using fallback');
  return [];
}

/**
 * Safe wrapper for delete operation that provides a fallback
 * @returns {Object} Delete operation or fallback
 */
function getFieldValueDelete() {
  if (admin.firestore && admin.firestore.FieldValue && admin.firestore.FieldValue.delete) {
    return admin.firestore.FieldValue.delete();
  }
  // Fallback for testing environments
  console.warn('Warning: fieldValue.delete not available, using fallback');
  return null;
}
  ...
  try {
    ...
    if (!configDoc.exists) {
      ...
      await configRef.update({
        delegatesList: getArrayUnion(principal),
        [`agentResources.${agent}`]: getArrayUnion(resource),
      });
    }
    ...
    if (!resourceDoc.exists) {
      ...
      await resourceRef.update({
        authorizedAgents: getArrayUnion(agent),
        authorizedPrincipals: getArrayUnion(principal),
...
async function revokeAgentAccess(principal, agent, resource) {
  try {
    ...
    const configRef = firestore.collection('config').doc('salleyport');
    await configRef.update({
      [`agentResources.${agent}`]: getArrayRemove(resource),
    });
    ...
    if (agentResources.length === 0) {
      ...
      await configRef.update({
        delegatesList: getArrayRemove(principal),
      });
    }
    ...
    const resourceRef = firestore.collection('resources').doc(resource);
    await resourceRef.update({
      authorizedAgents: getArrayRemove(agent),
      [`accessTypes.${agent}`]: getFieldValueDelete(),
      updatedAt: getServerTimestamp(),
    });
    ...
    if (authorizedAgents.length === 0) {
      ...
      await resourceRef.update({
        authorizedPrincipals: getArrayRemove(principal),
      });
    }
  ...
  try {
    ...
    if (!principalDoc.exists) {
      ...
      await principalRef.update({
        copilots: getArrayUnion(copilot),
      });
    }
...
async function unlinkCopilot(principal, copilot) {
  try {
    ...
    const principalRef = firestore.collection('principals').doc(principal.replace(/[^\w-]/g, ''));
    await principalRef.update({
      copilots: getArrayRemove(copilot),
    });
...
async function grantCopilotAccess(principal, copilot, resource, accessType = 'readonly') {
  try {
    ...
    await resourceRef.update({
      authorizedCopilots: getArrayUnion(copilot),
