...
async function unlinkCopilot(principal, copilot) {
  try {
    ...
    const accessQuery = firestore.collection('resourceAccess')
...
async function verifyCopilot(copilot, principal, culturalEmpathyCode) {
  try {
    ...
    const resourceQuery = firestore.collection('resources')
...
async function cleanupPRFix() {
  ...
  return await revokeAgentAccess(
    'pr@coaching2100.com',
    '001',
    'pr-2bd91160bf21ba21'
  );
