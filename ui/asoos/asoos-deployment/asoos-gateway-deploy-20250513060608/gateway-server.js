  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = https.request(
        MCP_CONFIG.gateway.endpoint + '/status', 
        requestOptions, 
        (res) => {
          let data = '';
          res.on('data', (chunk) => {
...
app.post('/api/sallyport/initiate', async (req, res) => {
  try {
    ...
    const challengeId = crypto.randomUUID ? crypto.randomUUID() : `challenge-${Date.now()}-${Math.random().toString(36).substring(2, 15)}`;
...
app.post('/api/request-key', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      expiresAt: result.expiresAt || new Date(Date.now() + (result.expiresIn || 3600) * 1000).toISOString(),
...
app.post('/webhook/model-update', (req, res) => {
  ...
  try {
    ...
    if (data.claude && data.claude.modelName) {
      console.log(`Updating Claude model from ${MCP_CONFIG.claude.modelName} to ${data.claude.modelName}`);
      MCP_CONFIG.claude.modelName = data.claude.modelName;
    }
    
    if (data.claude && data.claude.maxTokens) {
      console.log(`Updating max tokens from ${MCP_CONFIG.claude.maxTokens} to ${data.claude.maxTokens}`);
