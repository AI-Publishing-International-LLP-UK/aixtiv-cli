          ...
          res.on('end', () => {
            if (res.statusCode >= 200 && res.statusCode < 300) {
              try {
                ...
                console.log(`Warning: Failed to parse gateway response: ${e.message} - Using fallback mode`);
              ...
              console.log(`Warning: Gateway returned status code ${res.statusCode} - Using fallback mode`);
              gatewayConnected = false;
              pendingConnection = null;
              resolve({
                status: "fallback",
