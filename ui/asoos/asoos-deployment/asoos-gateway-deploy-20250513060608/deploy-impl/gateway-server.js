...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Service-ID', 'X-Client-ID', 'X-Request-ID']
})); 
...
app.post('/webhook/model-update', (req, res) => {
  ...
  try {
    ...
    if (data.claude && data.claude.modelName) {
      logger.info(`Updating Claude model from ${MCP_CONFIG.claude.modelName} to ${data.claude.modelName}`);
      MCP_CONFIG.claude.modelName = data.claude.modelName;
    }
    
    if (data.claude && data.claude.maxTokens) {
      logger.info(`Updating max tokens from ${MCP_CONFIG.claude.maxTokens} to ${data.claude.maxTokens}`);
    ...
    const server = app.listen(PORT, () => {
      ...
      process.on('uncaughtException', (error) => {
        ...
        if (error.message.includes('gateway') || error.message.includes('connection')) {
          gateway.validateGatewayConnection()
            .then(connected => {
              if (!connected) {
