...
// Configuration
const DOMAIN_CACHE_PATH = path.join(
...
function checkDomainMigrationStatus() {
  ...
  try {
    ...
    console.log(
      `${COLORS.green}Found ${allDomainsFile.length} domains in all-domains.txt file${COLORS.reset}`
    );
    ...
    console.log(
      `${COLORS.green}Found ${customDomainsFile.length} domains in custom-domains.txt file${COLORS.reset}`
    );

    // Combine domains from both files and remove duplicates
    const allDomains = [...new Set([...allDomainsFile, ...customDomainsFile])];
    console.log(
      `${COLORS.green}Total unique domains across both files: ${allDomains.length}${COLORS.reset}`
    );
    ...
    if (fs.existsSync(DOMAIN_CACHE_PATH)) {
      ...
      console.log(
        `${COLORS.green}Found ${cachedDomains.length} domains in Firebase domain cache${COLORS.reset}`
      );
    } else {
    ...
    const pendingDomainsFromFile = allDomains.filter((domain) => !cachedDomains.includes(domain));

    // Generate summary report
    console.log(`\n${COLORS.magenta}=== Migration Status Summary ===${COLORS.reset}`);
    console.log(
      `${COLORS.cyan}Domains in all-domains.txt:       ${COLORS.white}${allDomainsFile.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Domains in custom-domains.txt:    ${COLORS.white}${customDomainsFile.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Total unique domains:             ${COLORS.white}${allDomains.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Domains migrated to Firebase:     ${COLORS.white}${cachedDomains.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Domains pending migration:        ${COLORS.white}${pendingDomainsFromFile.length}${COLORS.reset}`
    );
    ...
    Object.keys(domainFamilies)
      .sort()
      .forEach((family) => {
        console.log(
          `${COLORS.yellow}${family}: ${COLORS.white}${domainFamilies[family].length} domains${COLORS.reset}`
        );
      });
    ...
    if (pendingDomainsFromFile.length > 0) {
      ...
      if (pendingDomainsFromFile.length > 20) {
        console.log(
          `${COLORS.yellow}... and ${pendingDomainsFromFile.length - 20} more${COLORS.reset}`
        );
      }
    }
    ...
    if (pendingDomainsFromFile.length > 0) {
      console.log(
        `${COLORS.green}Run the following commands to import ${pendingDomainsFromFile.length} domains into cache:${COLORS.reset}`
      );
      console.log(
        `${COLORS.yellow}./scripts/bulk-domain-import.sh ${ALL_DOMAINS_FILE_PATH} brand api-for-warp-drive false${COLORS.reset}`
      );
      console.log(
        `${COLORS.yellow}./scripts/bulk-domain-import.sh ${CUSTOM_DOMAINS_FILE_PATH} brand api-for-warp-drive false${COLORS.reset}`
      );

      console.log(`\n${COLORS.green}Then provision SSL certificates with:${COLORS.reset}`);
      console.log(
        `${COLORS.yellow}./scripts/batch-ssl-provision.sh ${ALL_DOMAINS_FILE_PATH} firebase api-for-warp-drive false${COLORS.reset}`
      );
      console.log(
        `${COLORS.yellow}./scripts/batch-ssl-provision.sh ${CUSTOM_DOMAINS_FILE_PATH} firebase api-for-warp-drive false${COLORS.reset}`
      );
    } else {
      console.log(
        `${COLORS.green}All domains from both domain files appear to be migrated to Firebase.${COLORS.reset}`
      );
    ...
    console.error(
      `${COLORS.red}Error checking domain migration status: ${error.message}${COLORS.reset}`
    );
    if (error.stack) {
