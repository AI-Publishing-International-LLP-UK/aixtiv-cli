...
async function testEmbeddingGeneration() {
  ...
  try {
    ...
    if (dimensions !== 1536) {
      console.warn(
        '⚠️ Warning: Embedding dimension is not 1536 as expected for OpenAI ada-002 model'
      );
    }
...
async function testSearchSimilarMemories() {
  ...
  try {
    ...
    const searchResults = await searchSimilarMemories(searchQuery, { userId: 'test-user-123' }, 5);
...
async function testSearchSimilarPrompts() {
  ...
  try {
    ...
    const searchResults = await searchSimilarPrompts(searchQuery, { userId: 'test-user-123' }, 5);
