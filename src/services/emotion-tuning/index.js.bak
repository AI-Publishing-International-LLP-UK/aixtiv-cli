...
class EmotionTuningService {
  ...
  async updateUserPreferences(userId, preferences) {
    ...
    try {
      ...
      // Validate intensity
      if (
        preferences.toneIntensity !== undefined &&
  ...
  async adjustTone(message, toneType, intensity, options = {}) {
    ...
    try {
      ...
      const adjustedMessage = this._applyToneAdjustments(
        message,
        toneDefinition,
        originalSentiment
      );
  ...
  _applyToneAdjustments(message, toneDefinition, sentiment) {
    ...
    let adjustedSentences = sentences.map((sentence) => {
      ...
      const replacementsToApply = Math.floor(
        toneDefinition.keywords.replace.length * normalizedIntensity
      );
      ...
      if (normalizedIntensity > 0.5) {
        switch (toneDefinition.sentenceStructure) {
          ...
          case 'direct':
            ...
            adjustedSentence = adjustedSentence.replace(
              /I think that |In my opinion, |It seems that /gi,
              ''
            );
            break;
        }
  ...
  _determineIntent(message) {
    ...
    if (lowerMessage.includes('?')) {
      ...
      lowerMessage.includes('does not work') ||
