...
class OAuth2Service {
  ...
  async getUserAccessToken(userId, provider) {
    ...
    if (this.tokenCache.has(cacheKey)) {
      ...
      if (cachedToken.expiresAt > Date.now() + 60000) {
        // 1-minute buffer
      ...
      if (cachedToken.refreshToken) {
        try {
          ...
          console.error(
            `Failed to refresh token for user ${userId} and provider ${provider}:`,
            error.message
          );
    ...
    if (tokenData.refreshToken) {
      try {
        ...
        throw new Error(
          `OAuth2 token expired and refresh failed for user ${userId} and provider ${provider}`
        );
      }
    } else {
      throw new Error(
        `OAuth2 token expired and no refresh token available for user ${userId} and provider ${provider}`
      );
    }
  }
  ...
  async getServiceAccessToken(serviceName, provider) {
    ...
    if (this.tokenCache.has(cacheKey)) {
      ...
      if (cachedToken.refreshToken) {
        try {
          ...
          console.error(
            `Failed to refresh token for service ${serviceName} and provider ${provider}:`,
            error.message
          );
          // Fall through to fetch from storage
        }
    ...
    const tokenDoc = await this.db
      .collection('service_oauth_tokens')
      .doc(`${serviceName}_${provider}`)
      .get();
    ...
    try {
      ...
      if (tokenData.refreshToken) {
        try {
          ...
          throw new Error(
            `OAuth2 token expired and refresh failed for service ${serviceName} and provider ${provider}`
          );
        }
      } else {
        throw new Error(
          `OAuth2 token expired and no refresh token available for service ${serviceName} and provider ${provider}`
        );
      }
    } catch (error) {
