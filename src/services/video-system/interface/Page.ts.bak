...
import { FunctionLocator, Locator, NodeLocator, type AwaitedLocator } from './locators/locators.js';
...
export abstract class Page extends EventEmitter<PageEvents> {
  ...
  override on<K extends keyof EventsWithWildcard<PageEvents>>(
    ...
    if (wrapper === undefined) {
      ...
      this.#requestHandlers.set(handler as (event: PageEvents[PageEvent.Request]) => void, wrapper);
    }
    return super.on(type, wrapper as (event: EventsWithWildcard<PageEvents>[K]) => void);
  }
    ...
    if (type === PageEvent.Request) {
      handler =
        (this.#requestHandlers.get(
          handler as (event: EventsWithWildcard<PageEvents>[PageEvent.Request]) => void
   ...
   * `<script>` element.
   */
  async addScriptTag(options: FrameAddScriptTagOptions): Promise<ElementHandle<HTMLScriptElement>> {
  ...
  async addStyleTag(options: FrameAddStyleTagOptions): Promise<ElementHandle<HTMLLinkElement>>;
  async addStyleTag(
   ...
   */
  async screencast(options: Readonly<ScreencastOptions> = {}): Promise<ScreenRecorder> {
   ...
   *
   */
  async screenshot(options: Readonly<ScreenshotOptions> & { encoding: 'base64' }): Promise<string>;
  async screenshot(options?: Readonly<ScreenshotOptions>): Promise<Uint8Array>;
  @guarded(function () {
    return this.browser();
  })
  async screenshot(userOptions: Readonly<ScreenshotOptions> = {}): Promise<Uint8Array | string> {
