...
class SpeechCore {
  ...
  detectLanguage(text) {
    ...
    if (typeof text !== 'string') {
      console.warn('detectLanguage received non-string input:', typeof text);
      return this.config.languages.default;
    }
    ...
    // Calculate percentage of Spanish words (avoid division by zero)
    const spanishPercentage = words.length > 0 ? (spanishWordCount / words.length) * 100 : 0;
  ...
  async textToSpeech(text, options = {}) {
    ...
    try {
      ...
      const voiceName =
        options.name || options.voice || this.getBestVoice(languageCode, gender, quality);
      ...
      const voiceLanguageCode =
        voiceSettings.name.split('-')[0] + '-' + voiceSettings.name.split('-')[1];
