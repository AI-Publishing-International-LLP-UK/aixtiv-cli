...
const DOMAIN_STYLES = require('../../aixtiv').DOMAIN_STYLES || {};
...
function registerCommands(register) {
  ...
  if (DOMAIN_STYLES) {
    DOMAIN_STYLES.secrets = secretsStyle;
  }
  ...
  register(
    ...
    [
      ...
      {
        flags: '-o, --output <format>',
        description: 'Output format: raw, json, env (default: raw)',
      },
  ...
  register(
    ...
    [
      ...
      {
        flags: '-l, --labels <labels>',
        description: 'Secret labels (comma-separated key=value pairs)',
      },
    ...
    createCommand,
    [
      'api-key-openai --value=sk_123456789',
      'firebase-credentials --file=./firebase-key.json',
      'db-password --env-var=DB_PASSWORD',
    ]
  );
