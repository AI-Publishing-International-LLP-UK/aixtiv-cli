...
class Telemetry {
  ...
  async init() {
    ...
    try {
      if (!fs.existsSync(TELEMETRY_PATH)) {
        console.error(
          `Telemetry binary not found at ${TELEMETRY_PATH}. Telemetry will be disabled.`
        );
        return false;
      }
    } catch (error) {
    ...
    try {
      ...
      execFile(
        ...
        [
          ...
          commandName,
          '--error',
          errorMsg.substring(0, 200), // Truncate long error messages
        ],
        { detached: true }
      ).unref();
    } catch (error) {
  ...
  async shutdown() {
    ...
    try {
      execFile(TELEMETRY_PATH, ['shutdown', '--context-id', this.contextId], { timeout: 1000 });
      this.initialized = false;
    } catch (error) {
      // Silently fail - telemetry should never break the application
    }
...
module.exports = {
  ...
  recordDuration: (commandName, durationMs) =>
    getTelemetry().recordDuration(commandName, durationMs),
