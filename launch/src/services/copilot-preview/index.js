...
class CopilotPreviewService {
    ...
    try {
      ...
      for (const collection of collections) {
        try {
          ...
          console.warn(
            `Collection ${collection} does not exist. It will be created on first write.`
          );
        }
      }
  ...
  async createPreview(userId, copilotId, message, responseText, options = {}) {
    ...
    try {
      ...
      const messageSentiment =
        options.sentiment || (await this.speechService.analyzeSentiment(message));
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        previews.push({
          ...
          userMessage:
            data.userMessage.substring(0, 50) + (data.userMessage.length > 50 ? '...' : ''),
  ...
  _detectIntent(message) {
    ...
    if (
      ...
      messageLC.includes('why')
    ) {
      return 'Question';
    } else if (
      messageLC.includes('help') ||
      messageLC.includes('please') ||
      messageLC.includes('need')
    ) {
      return 'Request for assistance';
    } else if (
      messageLC.includes('thank') ||
      messageLC.includes('appreciate') ||
      messageLC.includes('grateful')
    ) {
      return 'Appreciation';
    } else if (
      messageLC.includes('problem') ||
      messageLC.includes('issue') ||
      messageLC.includes('error') ||
      messageLC.includes('wrong')
    ) {
  ...
  _generateReasoning(message, sentiment) {
    ...
    if (intent === 'Question') {
      ...
      reasoning +=
        'so acknowledging their gratitude and offering further assistance would be appropriate.';
    } else if (intent === 'Problem report') {
      reasoning +=
        'so validating their concern, showing understanding, and providing troubleshooting steps is important.';
    } else {
      reasoning +=
        'so acknowledging their information and building upon it would create a natural conversation flow.';
    }
  ...
  async _analyzeTone(text) {
    try {
      ...
      const tones = [
        {
          name: 'formal',
          keywords: ['therefore', 'furthermore', 'subsequently', 'accordingly', 'thus', 'hence'],
        },
        {
          name: 'friendly',
          keywords: ['hi', 'hello', 'thanks', 'appreciate', 'glad', 'happy', '!'],
        },
        {
          name: 'informative',
          keywords: ['information', 'data', 'research', 'analysis', 'report', 'statistics'],
        },
        {
          name: 'empathetic',
          keywords: ['understand', 'feel', 'sorry', 'empathize', 'difficult', 'challenging'],
        },
        {
          name: 'enthusiastic',
          keywords: ['amazing', 'exciting', 'fantastic', 'great', 'excellent', '!'],
        },
        {
          name: 'technical',
          keywords: ['system', 'function', 'process', 'technical', 'implementation', 'code'],
        },
        {
          name: 'reassuring',
          keywords: ['assure', 'confident', 'trust', 'reliable', 'guarantee', 'certain'],
        },
      ];
