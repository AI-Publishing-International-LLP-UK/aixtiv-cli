...
class SpeakerRecognitionService {
          ...
          doc: () => ({
            get: async () => ({
              ...
              data: () => ({
                ...
                enrolledPhrases: [
                  { phrase: 'Test phrase 1', enrollmentId: 'enrollment-1', timestamp: new Date() },
                  { phrase: 'Test phrase 2', enrollmentId: 'enrollment-2', timestamp: new Date() },
                ],
              }),
            }),
            set: async () => true,
  ...
  async initialize() {
    ...
    try {
      ...
      const apiConfig = await this.db.collection('api_config').doc('speaker_recognition').get();
    ...
    try {
      ...
      await this.db
        ...
        .update({
          ...
          enrollmentStatus:
            profile.enrolledPhrases.length + 1 >= this.config.minEnrollmentPhrases
              ? 'completed'
              : 'in_progress',
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
  ...
  async identifySpeaker(audio, profileIds = []) {
    ...
    try {
      ...
      await this.db
        .collection('speaker_identifications')
  ...
  async getProfileDetails(profileId) {
    ...
    try {
      ...
      const verificationsSnapshot = await this.db
        .collection('speaker_verifications')
  ...
  async deleteProfile(profileId) {
    ...
    try {
      ...
      const enrollmentsSnapshot = await this.db
        .collection('speaker_enrollments')
        .where('profileId', '==', profileId)
        .get();
