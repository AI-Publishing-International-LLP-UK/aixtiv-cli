...
async function initCommand(options) {
  try {
    ...
    if (options.validate !== false) {
      ...
      if (!isValid) {
        utils.ui.feedback.error(
          'Invalid GCP credentials. Please check your service account key file and project ID.'
        );
        return;
      }
    ...
    utils.ui.feedback.info(
      chalk.cyan(`  export GOOGLE_APPLICATION_CREDENTIALS="${config.keyFile}"`)
    );
  } catch (error) {
