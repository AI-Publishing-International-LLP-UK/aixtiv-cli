...
export function usePineconeSearch() {
  ...
  const search = useCallback(
    async (query: string, options: PineconeSearchOptions = {}): Promise<PineconeMatch[]> => {
      setLoading(true);
      setError(null);
  ...
  const searchMemories = useCallback(
    async (query: string, userId?: string, topK: number = 10): Promise<PineconeMatch[]> => {
  ...
  const searchPrompts = useCallback(
    async (query: string, agentId?: string, topK: number = 10): Promise<PineconeMatch[]> => {
