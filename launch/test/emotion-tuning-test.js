...
const testMessages = {
  positive:
    "I'm really impressed with this feature! It's exactly what I needed and works perfectly. Thank you for the help!",
  negative:
    "This is frustrating and doesn't work as expected. I've tried multiple times and keep getting errors. This needs to be fixed immediately.",
  neutral:
    'I need to adjust the settings for the project. Can you show me how to configure the system preferences and save the changes?',
  technical:
    "I'm getting a TypeError: Cannot read property 'length' of undefined in the processData function. How can I fix this issue?",
  complaint:
    "I've been waiting for over a week and still haven't received a response. This is unacceptable service.",
  request:
    'Could you please help me set up the environment as soon as possible? I need it urgently for my presentation tomorrow.',
};
...
async function runTests() {
  ...
  try {
    ...
    for (const toneType of Object.keys(availableTones)) {
      const result = await emotionService.adjustTone(testMessages.neutral, toneType, 7);
    ...
    const lowIntensity = await emotionService.adjustTone(testMessages.neutral, 'confident', 3);

    const highIntensity = await emotionService.adjustTone(testMessages.neutral, 'confident', 9);

    assert(
      lowIntensity.intensity !== highIntensity.intensity,
      'Different intensity levels should be recorded'
    );
    ...
    for (const [sentimentType, message] of Object.entries(testMessages)) {
      ...
      assert(
        ...
        'Intensity should be between 1-10'
      );
      assert(suggestion.explanation, 'Should include an explanation');
      console.log(
        `✓ Tone suggestion for ${sentimentType} message: ${suggestion.tone} (${suggestion.intensity}/10)`
      );
    }
    ...
    assert(
      feedback.adjustmentId === adjustmentResult.id,
      'Feedback should reference the adjustment'
    );
    ...
    const customTone = await emotionService.addCustomTone(testUserId, 'professional-casual', {
    ...
    assert(
      customTone.customToneRules.some((t) => t.name === 'professional-casual'),
      'Custom tone should be added'
    );
