...
dreamCommanderCommand
  ...
  .option(
    ...
    'Specific component to check (processor, router, classifier, channels)'
  )
    ...
    try {
      ...
      if (component) {
        try {
          ...
          const stats = componentModule.getStats
            ? componentModule.getStats()
            : { status: 'Unknown' };
      ...
      console.log(
        boxen(
          chalk.cyan('Message Processor:\n') +
            ...
            (detailed
              ? `Channel Counts: ${JSON.stringify(processorStats.channelCounts, null, 2)}\n`
              : ''),
          { padding: 1, margin: 1, borderColor: 'cyan' }
        )
      );
        ...
        console.log(
          boxen(
            chalk.cyan('Message Router:\n') +
              ...
              (detailed
                ? `Agent Counts: ${JSON.stringify(routerStats.agentCounts, null, 2)}\n`
                : ''),
            { padding: 1, margin: 1, borderColor: 'cyan' }
          )
        );
  ...
  .action(async (options) => {
    try {
      ...
      const currentConfig = configDoc.exists
        ? configDoc.data()
        : require('../../dream-commander/config/default.json').dreamCommander;
      ...
      if (get) {
        ...
        if (typeof value === 'object') {
          console.log(
            boxen(JSON.stringify(value, null, 2), { padding: 1, margin: 1, borderColor: 'green' })
          );
        } else {
          console.log(boxen(`${get} = ${value}`, { padding: 1, margin: 1, borderColor: 'green' }));
        }
      ...
      if (list) {
        ...
        console.log(
          boxen(JSON.stringify(currentConfig, null, 2), {
            padding: 1,
            margin: 1,
            borderColor: 'blue',
          })
    ...
    try {
      ...
      if (format === 'json') {
        ...
        for (const [channel, count] of Object.entries(stats.byChannel)) {
          channelTable.push([channel, count, `${Math.round((count / stats.total) * 100)}%`]);
        }
        ...
        for (const [status, count] of Object.entries(stats.byStatus)) {
          statusTable.push([status, count, `${Math.round((count / stats.total) * 100)}%`]);
        }
        ...
        for (const [priority, count] of Object.entries(stats.byPriority)) {
          priorityTable.push([priority, count, `${Math.round((count / stats.total) * 100)}%`]);
        }
  ...
  .action(async (options) => {
    try {
          ...
          boxen(
            chalk.cyan(`Message ID: ${id}\n`) +
              ...
              (message.classification
                ? `Classification: ${JSON.stringify(message.classification, null, 2)}\n\n`
                : '') +
      ...
      if (list) {
        ...
        let query = db
          .collection('dream_commander_messages')
        ...
        snapshot.forEach((doc) => {
          ...
          if (typeof message.content === 'string') {
            ...
            contentPreview =
              message.content.subject.substring(0, 50) +
              (message.content.subject.length > 50 ? '...' : '');
