  ...
  try {
    ...
    if (options.type) {
      filteredDomains = filteredDomains.filter(
        (domain) => (domain.type || '').toLowerCase() === options.type.toLowerCase()
      );
    }
    ...
    if (options.status) {
      filteredDomains = filteredDomains.filter(
        (domain) => (domain.status || '').toLowerCase() === options.status.toLowerCase()
      );
    }
    ...
    if (options.project) {
      filteredDomains = filteredDomains.filter(
        (domain) =>
          (domain.firebaseProject || domain.project || '').toLowerCase() ===
...
async function showDomainDetails(domainName, options = {}) {
  ...
  try {
    ...
    try {
      ...
      const cache = loadDomainCache();
      domainInfo = cache.domains.find((d) => d.name === domainName || d.domain === domainName);
...
async function registerDomain(domainName, options = {}) {
  ...
  try {
    ...
    // Update cache
    const cache = loadDomainCache();
    const domainIndex = cache.domains.findIndex(
