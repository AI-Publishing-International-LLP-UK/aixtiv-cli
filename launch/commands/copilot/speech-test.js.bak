...
const command = new Command('speech-test')
  ...
  .option(
    ...
    path.join(process.cwd(), 'speech-test.mp3')
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (audioContent && Buffer.isBuffer(audioContent) && audioContent.length > 0) {
        ...
        console.log(
          chalk.red('✗ Text-to-speech test failed - Invalid or empty audio content returned')
        );
      }
      ...
      console.log(
        `To play the generated audio, use your system's audio player to open: ${options.output}`
      );
    } catch (error) {
