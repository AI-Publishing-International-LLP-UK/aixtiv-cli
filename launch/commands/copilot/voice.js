 ...
 * Supports Google STT/TTS, personalization, and sentiment analysis.
 *
 * Features:
 * - Speech-to-Text (transcription)
 * - Text-to-Speech (voice generation)
 * - Voice personalization (pitch, rate, gender)
 * - Sentiment analysis
...
// Use the simplified implementation for voice commands
// This avoids hard-coding user IDs or specific copilot references
// and provides a clean interface for STT/TTS functionality
module.exports = require('./voice-simplified');
  ...
  try {
    ...
    if (result.sentiment) {
      ...
      const sentimentColor =
        result.sentiment.category === 'positive'
          ? 'green'
          : result.sentiment.category === 'negative'
            ? 'red'
            : 'yellow';
...
async function personalizeVoice(userId, copilotId, options) {
  ...
  if (options.pitch !== undefined || options.rate !== undefined || options.gender !== undefined) {
    settings = {
      ...
      gender: options.gender || 'FEMALE', // Google TTS doesn't support NEUTRAL gender
    };
  } else {
    ...
    const answers = await inquirer.prompt([
      {
        type: 'list',
        name: 'language',
        message: 'Select language and region:',
        choices: [
      ...
      },
      {
        type: 'list',
        name: 'voiceType',
        message: 'Select voice quality:',
        choices: [
          { name: 'Google Studio Voice (Premium)', value: 'studio' },
          { name: 'Google Neural2 Voice (High Quality)', value: 'neural2' },
          { name: 'Google Wavenet Voice (Good Quality)', value: 'wavenet' },
          { name: 'Google Standard Voice (Basic)', value: 'standard' },
        ],
        default: 'studio',
      },
      {
    ...
    // Use the speechCore's voice selection logic instead of hardcoding
    settings = {
      gender: answers.gender,
      pitch: answers.pitch,
      speakingRate: answers.speakingRate,
      languageCode: answers.language, // Selected language
      quality: answers.voiceType, // Selected voice quality
    };
  }
  ...
  try {
    ...
    // Personalized voice sample text based on copilot ID
    let copilotName = '';
    if (copilotId === 'copilot-01') {
      copilotName = 'SirHand';
    } else if (copilotId === 'copilot-02') {
      copilotName = 'QBLucy';
    } else {
      copilotName = `Copilot ${copilotId}`;
    }

    const testText = `This is a voice sample for ${copilotName} with personalized settings. I am using a ${settings.languageCode} ${settings.gender} voice with pitch ${settings.pitch} and speaking rate ${settings.speakingRate}.`;

    await generateSpeech(testText, {
      userId,
      copilotId,
      output: `${copilotId}-sample-${Date.now()}.mp3`,
    });
...
async function updateCEScore(userId, delta, reason) {
  ...
  try {
    ...
    // Update score
    const newScore = await speechService.updateCEScore(
      userId,
      numericDelta,
...
async function createDreamSession(userId, options) {
  ...
  try {
    ...
    console.log(
      `  aixtiv copilot voice --action dream-interact --sessionId ${session.sessionId} --message "Your message here"`
    );
...
async function interactWithDream(sessionId, message) {
  ...
  try {
    ...
    if (response.sentiment) {
      ...
      const sentimentColor =
        response.sentiment.category === 'positive'
          ? 'green'
          : response.sentiment.category === 'negative'
            ? 'red'
            : 'yellow';
