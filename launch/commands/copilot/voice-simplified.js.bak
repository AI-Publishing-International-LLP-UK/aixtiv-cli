...
async function recordAudio(duration = 10, outputPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (process.platform === 'darwin') {
      ...
      recorder = spawn('afrecord', ['-f', 'WAVE', '-c', '1', '-r', '16000', filePath]);
      ...
      default:
        console.log(
          chalk.yellow('\n⚠️  Please specify an action. Available actions:\n') +
            chalk.cyan('transcribe') +
            ' - Convert speech to text\n' +
            chalk.cyan('speak') +
            ' - Convert text to speech\n' +
            chalk.cyan('personalize') +
            ' - Set voice personalization settings\n' +
            chalk.cyan('test') +
            ' - Test voice functionality\n\n' +
            chalk.yellow('Example:') +
            ' aixtiv copilot voice --action speak --text "Hello" --copilotId your-copilot-id'
        );
    }
  } catch (error) {
...
async function handlePersonalize(userId, copilotId, voiceOptions = {}) {
  ...
  if (
    voiceOptions.pitch !== undefined ||
    voiceOptions.rate !== undefined ||
    voiceOptions.gender !== undefined
  ) {
    settings = {
...
async function handleTest(userId, copilotId) {
  ...
  try {
    ...
    const testText =
      'This is a test of the speech system. If you can hear this, the test was successful.';
