  ...
  client.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (config.__retryCount < MAX_RETRIES) {
        ...
        const delay =
          config.__retryDelay || BASE_RETRY_DELAY * Math.pow(2, config.__retryCount - 1);
...
function getGoDaddyCredentials() {
  ...
  const keyfilePath =
    process.env.GODADDY_API_KEYFILE ||
  ...
  throw new Error(
    'GoDaddy API credentials not found. Please set GODADDY_API_KEY and GODADDY_API_SECRET environment variables, or create a keyfile at ~/.aixtiv/godaddy-api-key.json'
  );
}
...
function logGoDaddyApiRequest(config) {
  if (process.env.AIXTIV_VERBOSE === 'true') {
    console.log(
      `[GoDaddy API Request] ${config.method.toUpperCase()} ${config.baseURL}${config.url}`
    );
...
function logGoDaddyApiResponse(response) {
  if (process.env.AIXTIV_VERBOSE === 'true') {
    console.log(
      `[GoDaddy API Response] ${response.status} ${response.statusText} - ${response.config.method.toUpperCase()} ${response.config.url}`
    );
...
// Export the GoDaddy API client functions
module.exports = {
  createGoDaddyClient,
  listDomains,
  getDomain,
