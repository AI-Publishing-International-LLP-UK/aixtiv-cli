...
function createDomainClient(options = {}) {
  ...
  switch (env) {
    case 'dev':
      baseURL =
        process.env.DOMAIN_API_DEV_ENDPOINT ||
        `https://domain-api-dev-dot-aixtiv-domain-mgmt.${region}.r.appspot.com`;
      break;
    case 'staging':
      baseURL =
        process.env.DOMAIN_API_STAGING_ENDPOINT ||
        `https://domain-api-staging-dot-aixtiv-domain-mgmt.${region}.r.appspot.com`;
      break;
    case 'prod':
    default:
      baseURL =
        process.env.DOMAIN_API_ENDPOINT ||
        `https://domain-api.${region}-c.aixtiv-domain-mgmt.cloud.goog`;
      break;
  }
  ...
  client.interceptors.request.use(
    async (config) => {
      ...
      if (process.env.MODEL_CONTEXT_PROTOCOL === 'enabled') {
        ...
        config.headers['X-Claude-Automation-Session'] =
          process.env.CLAUDE_AUTOMATION_SESSION || 'default';
      }
  ...
  client.interceptors.response.use(
    ...
    async (error) => {
      ...
      if ((response && [400, 401, 403, 404, 422].includes(response.status)) || config.__skipRetry) {
        return Promise.reject(error);
      }
      ...
      if (config.__retryCount < MAX_RETRIES) {
        ...
        console.log(
          `Retrying request (${config.__retryCount}/${MAX_RETRIES}) after ${delay}ms: ${config.method.toUpperCase()} ${config.url}`
        );
...
function getApiKey() {
  ...
  const keyfilePath =
    process.env.AIXTIV_API_KEYFILE ||
  ...
  throw new Error(
    'API key not found. Please set AIXTIV_API_KEY or AIXTIV_DOMAIN_API_KEY environment variable, or create a keyfile at ~/.aixtiv/api-key.json'
  );
}
...
function logApiResponse(response) {
  if (process.env.AIXTIV_VERBOSE === 'true') {
    console.log(
      `[API Response] ${response.status} ${response.statusText} - ${response.config.method.toUpperCase()} ${response.config.url}`
    );
 ...
 * @param {string} updates.type - Domain type
 * @param {string} updates.firebaseProject - Firebase project ID
 * @param {string} updates.expiryDate - Expiry date (YYYY-MM-DD)
 * @param {Object} options - Options
 * @param {string} options.env - Environment (dev, staging, prod)
 * @returns {Promise<Object>} Operation result
 */
async function updateDomain(domainName, updates, options = {}) {
  if (!domainName) {
    throw new Error('Domain name is required');
  }

  if (updates.type && !DOMAIN_TYPES.includes(updates.type)) {
    throw new Error(`Invalid domain type. Must be one of: ${DOMAIN_TYPES.join(', ')}`);
  }
  ...
  // Validate expiry date format if provided
  if (updates.expiryDate && !/^\d{4}-\d{2}-\d{2}$/.test(updates.expiryDate)) {
    throw new Error('Expiry date must be in YYYY-MM-DD format');
  }

  try {
    logAgentAction('domain-update', {
      domain: domainName,
      updates: { ...updates },
    });

    const response = await client.put(`/domains/${encodeURIComponent(domainName)}`, updates);
    ...
    throw new Error(`Failed to update domain ${domainName}: ${error.message}`);
  }
}
 ...
 * @returns {Promise<Object>} Operation result
 */
async function deleteDomain(domainName, options = {}) {
  if (!domainName) {
    throw new Error('Domain name is required');
  }
  ...
  try {
    logAgentAction('domain-delete', { domain: domainName });

    const response = await client.delete(`/domains/${encodeURIComponent(domainName)}`);
    ...
    throw new Error(`Failed to delete domain ${domainName}: ${error.message}`);
  }
}

// Export the Domain API client
module.exports = {
  listDomains,
  getDomain,
  addDomain,
  ...
  // Export godaddy provider
  godaddy: require('./providers/godaddy'),
};
