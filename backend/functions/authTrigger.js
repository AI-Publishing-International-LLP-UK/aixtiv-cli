...
const GATEWAY_API_URL = functions.config().gateway?.url || 'https://integration-gateway.aixtiv-symphony.com/validate-auth';
...
exports.onUserCreated = functions
  ...
  .onCreate(async (user, context) => {
    ...
    try {
      ...
      try {
        await db.collection('pilots').doc(user.uid).set({
          email: user.email,
 ...
 */
exports.processExistingUser = functions
  .region('us-west1')
  .https
