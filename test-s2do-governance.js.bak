...
async function runS2DOTest() {
  ...
  try {
    ...
    const workflowResult = await s2do.createWorkflow('Test Governance Workflow', 'approval', {
      createdBy: 'test-user',
      certificate: true,
    ...
    if (!workflowResult.success) {
      throw new Error(
        `Workflow creation failed: ${workflowResult.message || workflowResult.error}`
      );
    }
    ...
    console.log(
      `- Blockchain Verifiable: ${workflowResult.workflow.blockchainVerifiable || false}`
    );
    ...
    const approvalResult = await s2do.approveWorkflowStep(workflowId, 'step-1', 'reviewer-1', {
    ...
    console.log(
      `- Blockchain Verifiable: ${approvalResult.approval.blockchainVerifiable || false}`
    );
    ...
    const getWorkflowResult = await s2do.getWorkflow(workflowId, {
      verifyOnChain: true,
    ...
    if (!getWorkflowResult.success) {
      throw new Error(
        `Workflow retrieval failed: ${getWorkflowResult.message || getWorkflowResult.error}`
      );
    }
    ...
    if (getWorkflowResult.workflow.auditTrail) {
      ...
      getWorkflowResult.workflow.auditTrail.forEach((record, index) => {
        console.log(
          `  ${index + 1}. ${record.action} by ${record.performedBy} at ${record.timestamp}`
        );
      });
    }
