...
async function generateDomainReport() {
  ...
  try {
    ...
    const filteredGodaddyDomains = godaddyDomains.filter(
      (domain) => !EXCLUDED_DOMAINS.some((excluded) => domain.domain.endsWith(excluded))
    );

    console.log(
      `${COLORS.green}Found ${filteredGodaddyDomains.length} active domains in GoDaddy account${COLORS.reset}`
    );
    ...
    try {
      ...
      console.log(
        `${COLORS.green}Found ${firebaseSites.length} sites in Firebase Hosting${COLORS.reset}`
      );
    } catch (error) {
      console.log(
        `${COLORS.red}Could not fetch Firebase Hosting sites: ${error.message}${COLORS.reset}`
      );
    ...
    const pendingDomainsFromFile = allDomainsFile.filter(
      (domain) => !cachedDomains.includes(domain)
    );

    // Generate report
    console.log(
      `${COLORS.cyan}Total GoDaddy active domains:       ${COLORS.white}${filteredGodaddyDomains.length}${COLORS.reset}`
    );
    console.log(
    ...
    console.log(
      `${COLORS.cyan}Domains pending migration (GoDaddy): ${COLORS.white}${notMigratedDomains.length}${COLORS.reset}`
    );
    console.log(
      `${COLORS.cyan}Domains pending migration (file):   ${COLORS.white}${pendingDomainsFromFile.length}${COLORS.reset}`
    );
    ...
    if (notMigratedDomains.length > 0) {
      console.log(
        `${COLORS.green}Run the following command to import domains into cache:${COLORS.reset}`
      );
      console.log(
        `${COLORS.yellow}./scripts/bulk-domain-import.sh ${DOMAINS_FILE_PATH} brand api-for-warp-drive false${COLORS.reset}`
      );

      console.log(`\n${COLORS.green}Then provision SSL certificates with:${COLORS.reset}`);
      console.log(
        `${COLORS.yellow}./scripts/batch-ssl-provision.sh ${DOMAINS_FILE_PATH} firebase api-for-warp-drive false${COLORS.reset}`
      );
    } else {
      console.log(
        `${COLORS.green}All domains from GoDaddy appear to be migrated to Firebase.${COLORS.reset}`
      );
